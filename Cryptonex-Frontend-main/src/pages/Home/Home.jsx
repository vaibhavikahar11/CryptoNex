...
import { MessageCircle, LayoutGrid, List, TrendingUp, BarChart2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { CoinHeatmap } from "@/components/Heatmap/CoinHeatmap";
...
const Home = () => {
  ...
  // Fetch coins for "all" category only
  useEffect(() => {
    if (category === "all") {
    ...
    <div className="relative">
      ...
      <div className="lg:flex flex-col lg:flex-row">
        <div className="w-full p-6 sm:p-10 flex flex-col flex-grow">
          ...
          <div className="mb-6 fade-in-up">
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: "rgba(148,163,184,0.6)", marginTop: 2 }}>Simulated trading terminal</p>
              </div>
            </div>
            <PortfolioOverview />
          </div>
          ...
          {/* Category Buttons + Heatmap Toggle */}
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            {(category === "all" || category === "top50") && (
              <Button
                ...
                className="rounded-full ml-auto neon-btn flex items-center gap-2"
                style={{ fontSize: 12 }}
              >
                {showHeatmap ? <List size={14} /> : <LayoutGrid size={14} />}
            ...
            <>
              {showHeatmap ? (
                <div className="fade-in-up">
                  <CoinHeatmap coins={category === "all" ? coin.coinList : (coin.top50 || [])} />
                </div>
              ) : (
                <AssetTable
                  category={category}
                  ...
                  {item.role === "user" ? (
                    ...
                    <div className="w-full">
                      <div className="bg-slate-700 flex gap-2 py-4 px-4 rounded-md min-w-[15rem] w-full text-white">
                        {item.ans ? (
                          <div 
                            className="prose prose-invert prose-p:leading-relaxed prose-pre:p-0 prose-a:text-blue-400 prose-a:underline hover:prose-a:text-blue-300 w-full"
                            dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(marked.parse(item.ans)) }} 
                          />
                        ) : (
