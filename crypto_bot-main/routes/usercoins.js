...
router.post("/create", async (req, res) => {
  try {
    ...
    // Expect JSON payload
    const { coin_name, coin_symbol, price, quantity, volume, coin_picture } =
      req.body;
    ...
    if (!coin_name || !coin_symbol || !price || !quantity || !volume) {
      return res
        ...
        .json({
          error:
            "All fields (coin_name, coin_symbol, price, quantity, volume) are required.",
        });
    }
        ...
        blockchain
          ...
          .then((blockchainId) => {
            ...
            db.run(updateQuery, [blockchainId, this.lastID], (err2) => {
              if (err2) {
                console.error(
                  "Error updating coin with blockchain ID:",
                  err2.message,
                );
                return res
                  .status(500)
                  .json({
                    error: "Coin created but failed to update blockchain info.",
                  });
              }
          ...
          .catch((error) => {
            ...
            return res
              .status(500)
              .json({ error: "Coin created but blockchain minting failed." });
          });
      },
    );
  } catch (error) {
